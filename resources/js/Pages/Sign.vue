        ...
        <div class="card">
          <div class="card-body" style="background: darkgray">
            ...
            <form action="" class="card-img-top" @submit.prevent="sign" method="POST" autocomplete="off" >
              ...
              <vueSignature ref="signature" id="sig" :sigOption="option" :disabled="disabled" :defaultUrl="dataUrl">
              </vueSignature>
            </form>
...
export default {
  ...
  methods: {
    ...
    sign() {
      ...
      let currentObj = this;
      this.axios.post(route('sign'), { user_image: png }).then((result) => {
        

        this.axios.get(route('books')).then((response) => {
    ...
    deleteBook(id) {
      this.axios
        .post(`http://localhost:8000/web/store_image/delete/${id}`)
        .then((response) => {
