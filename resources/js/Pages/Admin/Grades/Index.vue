                          ...
                          <div class="card px-2 py-2">
                            <div class="row bg-color-blue mx-3 mb-4 py-1">
                                <div class="card-header">
                                    <h3 class="card text-center col-lg-12 text-white" style="background-color:#4851ec"><strong>Liste des fonctions</strong></h3>
                                        ...
                                        <tbody>
                                            <tr v-for="(grade, index) in grades" :key="index">
                                                ...
                                                <td class="text-center" v-if="$page.props.auth.hasRole.superAdmin || $page.props.auth.hasRole.admin">
                                                    <button class="btn btn-xs btn-primary" style="letter-spacing: 0.1em;" @click="editModal(grade)">
                                                        <i class="fa fa-pencil"></i>
                                                    </button>
                                                    &nbsp; &nbsp;
                                                    <button class="btn btn-xs btn-danger" @click="deleteGrade(grade)">
                                ...
                                <form @submit.prevent="checkMode">
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="name" class="h5">Nom de la fonction</label>
                                            <input type="text" class="form-control" id="grade" placeholder="Nom de la fonction" v-model="form.name" :class="{ 'is-invalid' : form.errors.name }" autofocus="autofocus" autocomplete="off" minlength="3" maxlength="35">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <label for="description" class="h5">Description de la fonction</label>
                                            <textarea rows="4"  class="form-control" id="description" v-model="form.description" :class="{ 'is-invalid' : form.errors.description }" autocomplete="off" placeholder="Description de la fonction" minlength="10" maxlength="255"></textarea>
                                        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'Ajouter nouvelle fonction' : 'Modifier la fonction';
            },
            gradeText() {
                return this.grades_count <= 1 ? 'Fonction' : 'Fonctions';
            },
            buttonTxt() {
                return this.editedIndex === -1 ? 'Ajouter' : 'Modifier';
            },
            checkMode() {
                return this.editMode === false ? this.createGrade: this.editGrade;
            }
        },
        methods: {
            editModal(grade) {
                this.editMode = true
                $('#modal-lg').modal('show')
                this.editedIndex = this.grades.indexOf(grade)
                this.form.name = grade.name
                this.form.id = grade.id
                this.form.description = grade.description
            },
            openModal() {
                this.editedIndex = -1
                $('#modal-lg').modal('show')
            ...
            createGrade() {
                this.form.post(this.route('admin.grades.store'), {
                    preserveScroll: true,
                    onSuccess:() => {
                        this.closeModal()
                        Toast.fire({
                            icon: 'success',
                            title: 'Nouvelle fonction créée avec succès !'
                        })
                    }
                })
            },
            editGrade() {
                this.form.patch(this.route('admin.grades.update', this.form.id, this.form), {
                    preserveScroll: true,
                    onSuccess:() => {
                        Toast.fire({
                            icon: 'success',
                            title: 'La fonction a été modifiée avec succès !'
                        })
                        this.closeModal()
                    }
                })
            ...
            deleteGrade(grade) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        this.form.delete(this.route('admin.grades.destroy', grade), {
                            preserveScroll: true,
                            onSuccess: ()=> {
                                Swal.fire(
                                    'Supprimée!',
                                    'La fonction a bien été supprimée',
                                    'success'
                                )
                            }
                        })
