                          ...
                          <div class="card px-2 py-2">
                            <div class="row bg-color-blue mx-3 mb-4 py-1">
                                <div class="card-header">
                                    <h3 class="card text-center col-lg-12 text-white" style="background-color:#4851ec"><strong>Liste des départements</strong></h3>
                                    ...
                                    <table id="myTable" class="table table-striped table-bordered dataTable" cellspacing="0" width="100%" role="grid" aria-describedby="example_info" style="width: 100%;">
                                        ...
                                        <tbody>
                                            <tr v-for="(department, index) in departments" :key="index">
                                                ...
                                                <td>{{ department.description.substring(0, 25) + '...' }}</td>
                                                <td>{{ department.created_at }}</td>
                                                <td class="text-center" v-if="$page.props.auth.hasRole.superAdmin || $page.props.auth.hasRole.admin">
                                                    <button class="btn btn-xs btn-secondary" style="letter-spacing: 0.1em;" @click="showDepartment(department)">
                                                        <i class="fa fa-eye"></i>
                                                    </button>
                                                    &nbsp; &nbsp;
                                                    <button class="btn btn-xs btn-primary" style="letter-spacing: 0.1em;" @click="editModal(department)">
                                                        <i class="fa fa-pencil"></i>
                                                    </button>
                                                    &nbsp; &nbsp;
                                                    <button class="btn btn-xs btn-danger" @click="deleteDepartment(department)">
                                ...
                                <form @submit.prevent="checkMode">
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="name" class="h5">Nom du département</label>
                                            <input type="text" class="form-control" id="department" placeholder="Nom du département" v-model="form.name" :class="{ 'is-invalid' : form.errors.name }" autofocus autocomplete="off" minlength="3" maxlength="35">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <label for="description" class="h5">Description du département</label>
                                            <textarea rows="4"  class="form-control" id="description" v-model="form.description" :class="{ 'is-invalid' : form.errors.description }" autocomplete="off" placeholder="Description du département" minlength="10" maxlength="255"></textarea>
                                        </div>
...
<script>
    ...
    export default {
        props: ['departments', 'departments_count'],
        components: {
            AdminLayout,
        },
        data() {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'Ajouter nouveau département' : 'Modifier le département';
            },
            departText() {
                return this.departments_count <= 1 ? 'Département' : 'Départements';
            },
            buttonTxt() {
                return this.editedIndex === -1 ? 'Ajouter' : 'Modifier';
            },
            checkMode() {
                return this.editMode === false ? this.createDepartment: this.editDepartment;
            }
        },
        methods: {
            ...
            createDepartment() {
                this.form.post(this.route('admin.departments.store'), {
                    ...
                    onSuccess:() => {
                        ...
                        Toast.fire({
                            icon: 'success',
                            title: 'Nouveau département créé avec succès !'
                        })
                    }
                })
            },
            editDepartment() {
                this.form.patch(this.route('admin.departments.update', this.form.id, this.form), {
                    preserveScroll: true,
                    onSuccess:() => {
                        Toast.fire({
                            icon: 'success',
            ...
            showDepartment(department){
                this.form.get(this.route('admin.departments.show', department));
            },
            deleteDepartment(department) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        this.form.delete(this.route('admin.departments.destroy', department), {
                            preserveScroll: true,
                            onSuccess: ()=> {
                                Swal.fire(
                                    'Supprimée!',
